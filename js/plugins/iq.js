 ...
 */
const IqPlugin = function (framework) {
  ...
  const contentType = 'ooyala';
  let playingInstreamAd = false;
  let iqEnabled = false;
  let allowThrift = false;
  let thriftPcode = null;
  let jsonPcode = null;

  let adFirstQuartile = false;
  let adSecondQuartile = false;
  let adThirdQuartile = false;
  let adLastQuartile = false;

  let currentPlayhead = 0;
  let lastReportedPlayhead = 0;
  let adOffset = 0;
  let adTimeline = [];

  let playingSsaiAd = false;
  let ssaiAdTransition = false;
  ...
  this.processAdTimeline = function (timeline) {
    ...
    for (let index = 0; index < timeline.length; index++) {
      const durationMillis = Math.floor(timeline[index].duration * 1000);
      const startTimeMillis = Math.floor(timeline[index].start * 1000);
  ...
  this.reportAdPlaythrough = function (eventName, playhead, duration) {
    ...
    if (OO._.isFinite(playhead) && playhead >= 0 && OO._.isFinite(duration) && duration > 0) {
      const playheadMillis = Math.floor(playhead * 1000);
  ...
  this.processEvent = function (eventName, params) {
    ...
    switch (eventName) {
      ...
      case OO.Analytics.EVENTS.SSAI_PLAY_SINGLE_AD:
        ...
        if (!foundAd) {
          const durationMillis = Math.floor(params[0].ad.duration * 1000);
          const adData = {
    ...
    switch (eventName) {
        ...
        if (params && params[0] && params[0].streamPosition > 0) {
          if (playingInstreamAd || playingSsaiAd) {
            ...
            if (OO._.isFinite(currentTime) && currentTime > 0) {
              ...
              const offsetPlayhead = currentPlayhead - adOffset;
      ...
      case OO.Analytics.EVENTS.VIDEO_SEEK_COMPLETED:
        lastReportedPlayhead = 0;
        if (params && params[0]) {
          const newPlayhead = Math.floor(params[0].timeSeekedTo * 1000);
